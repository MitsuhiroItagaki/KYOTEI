...
model_name = "tank"
input_table_name = "main.kyotei_db.predict_tan"
output_table_path = "/FileStore/batch-inference/tank"

# COMMAND ----------

# load table as a Spark DataFrame
#table = spark.table(input_table_name)
table = sql("select * , predict_tan as predict from main.kyotei_db.predict_tan")
...
output_df = table.withColumn("predict_tank", predict(struct(*table.columns)))
...
# MAGIC ```python
# MAGIC output_table = "" # Example: "ml.batch-inference.tank"
...
output_df.createOrReplaceTempView('predict_tank')
...
# MAGIC sum(wink) / (count(1) * 100) as `利益率`
# MAGIC from 
# MAGIC (
# MAGIC select
# MAGIC   RACEDATE,
...
# MAGIC   case when cast(predict_tan as int) in (cast(tan as int)) then tank 
...
# MAGIC   fuku1,
# MAGIC   fuku2,
# MAGIC   case when cast(predict_tan as int) in (cast(fuku1 as int), cast(fuku2 as int) ) then 1 else 0 end as win,
# MAGIC   case when cast(predict_tan as int) in (cast(fuku1 as int)) then fuku1k 
# MAGIC        when cast(predict_tan as int) in (cast(fuku2 as int)) then fuku2k 
# MAGIC   else 0 end as wink
# MAGIC from
# MAGIC   predict_tank
# MAGIC   where tan <> 'NaN' 
